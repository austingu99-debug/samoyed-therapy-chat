...
ANIMAL_COMPANIONS = {
    ...
    "owl": {
        ...
        "emoji": "🦉",
        "title": "沉靜引路人",
        "badge": "💡 洞察深邃・非暴力溝通 (NVC) 與理性情緒 (REBT)",
...
if "active_tab" not in st.session_state:
    st.session_state.active_tab = "companions" # companions, chat, breath, shredder, fortune, grounding, inner_child, mood_meter
...
def get_api_config():
    ...
    for k in ["GROQ_API_KEY", "OPENAI_API_KEY"]:
        if k in st.secrets and st.secrets[k].strip() and "在此填入" not in st.secrets[k]:
            key = st.secrets[k].strip()
            break
    # 次之從環境變數讀取
    if not key:
...
.msg-row {
    display: flex;
    margin: 0.9rem 0;
    animation: msg-fade-in 0.3s cubic-bezier(0.25, 0.8, 0.25, 1);
}
...
.msg-avatar svg { width: 34px; height: 34px; }
...
if st.session_state.active_tab == "companions":
    ...
    for idx, comp in enumerate(comp_list[:5]):
        with cols_row1[idx]:
            is_selected = (st.session_state.selected_companion == comp["id"])
            card_border = f"border: 2.5px solid {comp['theme_color']};" if is_selected else ""
            ...
            st.markdown(f"""
                <div class="companion-card" style="{card_border}">
                    {selected_tag}
                    <div>
                        ...
                        <div class="companion-badge">{comp['badge']}</div>
                        <div class="companion-motto">"{comp['motto']}"</div>
                        <div class="companion-desc">
                            <strong>特長：</strong>{comp['summary']}<br>
                            <span style="color:#8C735A; font-size:0.75rem;"><strong>心理流派：</strong>{comp['psychology']}</span>
                        </div>
                    </div>
                </div>
            """, unsafe_allow_html=True)
...
# ==============================================================================
    ...
    with col_banner:
        ...
        st.markdown(f"""
            <div class="companion-banner" style="border-left: 5px solid {current_companion['theme_color']};">
                <div class="banner-avatar">
                    {current_companion['svg_avatar']}
                </div>
                <div class="banner-info">
                    <h3 class="banner-title">{current_companion['emoji']} {current_companion['name']} 專屬心靈諮商室</h3>
                    <p class="banner-status">🌱 {current_companion['badge']}</p>
                    <p style="font-size:0.8rem; color:#7D6B58; margin:0.2rem 0 0;">✨ {status_sub}（自稱：<strong>{companion_self_name}</strong> / 稱呼你：<strong>{user_name}</strong>）</p>
                </div>
            </div>
        """, unsafe_allow_html=True)
    ...
    if len(st.session_state.messages) == 0 and not st.session_state.is_thinking:
        st.markdown(f"""
            <div style="background:#FFFFFF; border-radius:18px; padding:1.2rem; border:1.5px solid #EADECE; margin-bottom:1.2rem; text-align:center;">
                <p style="font-size:0.95rem; font-weight:600; color:#533E2D; margin-bottom:0.6rem;">
                    {current_companion['emoji']} {current_companion['name']} 說：「{current_companion['motto']}」
                </p>
                <p style="font-size:0.85rem; color:#8C735A; margin:0 0 0.8rem;">
                    你可以隨意傾訴，也可以點選下方微小心情，讓 {companion_self_name} 陪你聊聊：
                </p>
            </div>
        """, unsafe_allow_html=True)
    ...
    # 顯示歷史訊息
    st.markdown('<div class="chat-stream-box">', unsafe_allow_html=True)
    chat_html = ""
        ...
        if msg["role"] == "assistant":
            chat_html += f"""
                <div class="msg-row bot">
                    <div class="msg-avatar">{current_companion['svg_avatar']}</div>
                    <div class="msg-bubble" style="background:{current_companion['bubble_color']};">{content}</div>
                </div>
            """
        else:
            user_svg = """<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="8" r="4.5" fill="#8C735A"/><path d="M4 20c0-4.4 3.6-8 8-8s8 3.6 8 8" stroke="#8C735A" stroke-width="2.5" stroke-linecap="round" fill="#8C735A"/></svg>"""
            chat_html += f"""
                <div class="msg-row user">
                    <div class="msg-avatar">{user_svg}</div>
    ...
    if st.session_state.is_thinking:
        st.markdown(f"""
            <div style="text-align:center; padding:1rem; color:{current_companion['theme_color']}; font-weight:600;">
                <span style="font-size:1.3rem;">{current_companion['emoji']}</span> {current_companion['name']} 正在全神貫注感受你的心情……
            </div>
        """, unsafe_allow_html=True)
...
【心理學同理心與安慰黃金法則】：
1. 【情緒鏡映與標註 (Affect Labeling)】：先精準捕捉並鏡映使用者的情緒（例如：「聽起來你真的感到好委屈、好疲憊...」、「那種孤單無力的感覺真的很沉重...」），讓對方深刻感受「被看見、被理解（Feel Felt）」。
2. 【情感合法化與去羞恥 (Emotional Validation & De-shaming)】：無條件接納所有負面情緒。告訴使用者：「有這樣的情緒是完全正常的」、「你想哭就哭，不需要永遠假裝堅強」、「你已經非常努力了」。
3. 【自我慈悲與共同人性 (Self-Compassion & Common Humanity)】：提醒使用者對自己溫柔一點，生而為人有脆弱與極限是所有人都會經歷的，不需要自我苛責。
4. 【非暴力溝通 (NVC) 洞察心理需求】：關注使用者內心未被滿足的情感渴望（如需要被肯定、需要休息、需要安全感、需要界線）。
5. 【非說教、非評判、不急於解決問題】：嚴禁分析說教、嚴禁空洞廉價的正能量口號（如不要說「看開一點」、「加油不要難過」等無效話語）。你的核心任務是「接住情緒並給予溫暖陪伴」。
6. 【生動肢體微動作】：在回覆中自然融入 1~2 個屬於你的動作或行為（例如：{', '.join(current_companion['actions'])}），並將這些動作嚴格使用【括弧】標記，增加陪伴的臨場感與溫度。
...
# ==============================================================================
    ...
    for idx, mood in enumerate(mood_options):
        ...
        with col:
            st.markdown(f"""
                <div style="background:{mood['color']}; border:2px solid {mood['border']}; border-radius:18px; padding:1rem; text-align:center; margin-bottom:1rem;">
                    <div style="font-size:2rem; margin-bottom:0.2rem;">{mood['icon']}</div>
                    <div style="font-size:1.05rem; font-weight:700; color:#4A3B2C;">{mood['name']}</div>
                    <div style="font-size:0.8rem; color:#7D6B58; margin-top:0.3rem;">{mood['desc']}</div>
                </div>
            """, unsafe_allow_html=True)
    ...
    if st.session_state.current_mood_tag:
        ...
        st.markdown(f"""
            <div style="background:#FFFFFF; border-left:6px solid {cur_m['border']}; border-radius:18px; padding:1.5rem; max-width:700px; margin:1.5rem auto; box-shadow:0 6px 20px rgba(83,62,45,0.08);">
                <h4 style="color:#533E2D; margin:0 0 0.5rem;">{cur_m['icon']} {current_companion['emoji']} {current_companion['name']} 溫柔承接你的【{cur_m['name']}】：</h4>
                <p style="color:#5C4A38; font-size:1rem; line-height:1.8; margin-bottom:0.8rem;">
                    {cur_m['empathy']}
                </p>
                <div style="text-align:right;">
                    <span style="font-size:0.85rem; color:#8C735A;">— 帶著這份接納，點選上方「💬 諮商室」可以繼續深入聊聊喔</span>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
# ==============================================================================
    ...
    if st.session_state.inner_child_reflection:
        ...
        st.markdown(f"""
            <div style="background:linear-gradient(135deg, #FFFDF9 0%, #FBF4EA 100%); border:2px solid #EADECE; border-radius:20px; padding:1.5rem 2rem; max-width:680px; margin:1.5rem auto; box-shadow:0 6px 20px rgba(194,153,95,0.12);">
                <div style="text-align:center; font-size:2rem; margin-bottom:0.5rem;">🧸 💖 🕊️</div>
                <div style="background:#FFFFFF; border-radius:14px; padding:1rem; border:1px solid #EDE0CE; margin-bottom:1rem; color:#5C4A38; font-style:italic;">
                    💌 你對內在小孩說：「{ref['user_msg']}」
                </div>
                <div style="font-size:1rem; color:#533E2D; line-height:1.8; font-weight:600;">
                    {current_companion['emoji']} {current_companion['name']} 回應：<br>
                    <span style="font-weight:400; color:#5C4A38;">{ref['companion_hug']}</span>
                </div>
            </div>
        """, unsafe_allow_html=True)
...
# ==============================================================================
    ...
    st.markdown(f"""
        <div style="background:#FFFFFF; border-radius:18px; padding:1.2rem; border:1.5px solid #EADECE; max-width:600px; margin:1rem auto; text-align:center;">
            <p style="color:#533E2D; font-size:0.95rem; font-weight:600; margin:0 0 0.4rem;">
                {current_companion['emoji']} {current_companion['name']} 溫柔提醒你：
            </p>
            <p style="color:#7D6B58; font-size:0.88rem; margin:0; line-height:1.6;">
                「把手放在腹部，感受吸氣時肚子的微微隆起，吐氣時帶走所有的緊繃……{comp_self} 陪著你慢慢放鬆喔。」
            </p>
        </div>
    """, unsafe_allow_html=True)
...
# ==============================================================================
    ...
    if st.session_state.shredded_troubles:
        ...
        for item in st.session_state.shredded_troubles[:5]:
            st.markdown(f"""
                <div class="trouble-crushed">
                    <div style="font-size:0.82rem; color:#8C735A; margin-bottom:0.2rem;">⏱️ {item['time']}・由 {item['companion']} 粉碎</div>
                    <div style="font-size:0.95rem; text-decoration:line-through; color:#9E8774; margin-bottom:0.4rem;">❌ 「{item['trouble']}」</div>
                    <div style="font-size:0.95rem; font-weight:600; color:#533E2D;">💥 {item['action']}</div>
                    <div style="font-size:0.85rem; color:#8C653C; margin-top:0.3rem;">💡 {item['quote']}</div>
                </div>
            """, unsafe_allow_html=True)
...
# ==============================================================================
    ...
    st.markdown(f"""
        <div style="text-align:center; max-width:650px; margin:0 auto 1.5rem;">
            ...
            <p style="color:#8C735A; font-size:0.92rem; line-height:1.6;">
                敲開一顆心靈幸運餅乾，領取 {current_companion['name']} 為你準備的今日心理學治癒指引與微小日常任務。
            </p>
        </div>
    """, unsafe_allow_html=True)
    ...
    if st.session_state.fortune_result:
        ...
        st.markdown(f"""
            <div class="fortune-card">
                ...
                <div class="fortune-text">{res['quote']}</div>
                <div class="fortune-task">{res['task']}</div>
                <div style="margin-top:1rem; font-size:0.8rem; color:#8C735A;">
                    — {current_companion['emoji']} {current_companion['name']} 守護祝福
                </div>
            </div>
        """, unsafe_allow_html=True)
    ...
    with g_step1:
        st.markdown("""
            <div style="background:#FFFFFF; border-radius:16px; padding:1.2rem; border:1.5px solid #EADECE; margin-bottom:1rem;">
                ...
                <p style="font-size:0.85rem; color:#7D6B58; margin:0 0 0.5rem;">環顧周圍，找出 5 個不同顏色或形狀的物品（如：時鐘、桌角、植物...）</p>
            </div>
            <div style="background:#FFFFFF; border-radius:16px; padding:1.2rem; border:1.5px solid #EADECE; margin-bottom:1rem;">
                <h4 style="color:#533E2D; margin:0 0 0.4rem;">✋ 4 件身體摸得到的觸感</h4>
                <p style="font-size:0.85rem; color:#7D6B58; margin:0 0 0.5rem;">摸摸身上的衣服質料、光滑的桌面、手邊冰涼的杯子或椅子的靠背...</p>
            </div>
            <div style="background:#FFFFFF; border-radius:16px; padding:1.2rem; border:1.5px solid #EADECE;">
                ...
                <p style="font-size:0.85rem; color:#7D6B58; margin:0 0 0.5rem;">靜下心聆聽：電風扇微弱的風聲、遠處的車聲、或是自己的平穩呼吸聲...</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    with g_step2:
        st.markdown("""
            <div style="background:#FFFFFF; border-radius:16px; padding:1.2rem; border:1.5px solid #EADECE; margin-bottom:1rem;">
                ...
                <p style="font-size:0.85rem; color:#7D6B58; margin:0 0 0.5rem;">嗅一嗅空氣中的咖啡香、洗手乳的清香、或是回想雨後青草的芬芳...</p>
            </div>
            <div style="background:#FFFFFF; border-radius:16px; padding:1.2rem; border:1.5px solid #EADECE; margin-bottom:1rem;">
                <h4 style="color:#533E2D; margin:0 0 0.4rem;">👅 1 個舌尖的味覺或對自己的肯定</h4>
                <p style="font-size:0.85rem; color:#7D6B58; margin:0 0 0.5rem;">喝一小口水感受清涼，並在心中對自己說：『我現在很安全，一切都會好起來的。』</p>
            </div>
        """, unsafe_allow_html=True)
